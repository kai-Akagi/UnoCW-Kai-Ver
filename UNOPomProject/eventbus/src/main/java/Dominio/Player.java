 ...
 * </ul>
 *
 * <p><b>Principio de responsabilidad única:</b> Esta clase solo sabe sobre
 * el jugador y sus cartas. No sabe nada de la red, de la GUI, ni de las reglas.
 * Si necesitamos cambiar cómo se muestra un jugador en pantalla, no tocamos aquí.
...
public class Player {
    ...
    /**
     * Identificador del avatar elegido.
     * Corresponde al nombre del archivo de imagen en la carpeta de recursos.
     * Ejemplo: "avatar_1", "avatar_2", etc.
     */
    private final String avatarId;

    /**
     * Indica si este jugador es el Host (anfitrión) de la partida.
     * El Host tiene privilegios adicionales (iniciar partida, cerrar sala).
     */
    ...
    /**
     * Indica si el jugador marcó que está listo en el lobby de espera.
     * La partida no puede comenzar hasta que todos estén listos.
     */
     ...
     *
     * @param ready {@code true} para marcar como listo, {@code false} para desmarcar.
     */
    ...
    /**
     * Quita una carta específica de la mano del jugador y la devuelve.
     * Se usa cuando el jugador decide jugar una carta.
     *
    ...
    /**
     * Devuelve una vista de solo lectura de la mano del jugador.
     * Usamos una copia para que nadie pueda modificar la mano desde afuera.
     *
     * <p><b>Buena práctica:</b> Devolver una copia inmutable protege el estado
     * interno del objeto. Esto se llama "encapsulación" y evita errores difíciles
     * de rastrear donde alguien modifica la lista desde otro lugar.
     *
    ...
    /**
     * Verifica si el jugador puede jugar alguna carta de su mano
     * sobre la carta que está actualmente en la mesa.
     *
    ...
    public boolean hasPlayableCard(Card topCard) {
        for (Card card : hand) {
            if (card.canBePlacedOn(topCard)) return true;
        }
        return false;
    }

    /**
     * Verifica si este jugador ha ganado la partida.
     * En UNO, un jugador gana cuando se queda sin cartas.
     *
    ...
    /**
     * Verifica si el jugador tiene exactamente una carta en la mano.
     * En ese caso, debería gritar "UNO".
     *
     ...
     * Se usa al reiniciar una partida.
     */
