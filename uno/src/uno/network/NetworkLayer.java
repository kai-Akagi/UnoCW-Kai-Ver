...
/**
 * Maneja toda la comunicación por sockets entre Host y Peers. El Host abre el
 * servidor y distribuye los mensajes; los Peers se conectan y envían sus
...
public class NetworkLayer {
    ...
    public void registerGameListeners() {
        ...
        if (!session.isHost()) {
            eventBus.subscribe(uno.events.UnoPenaltyEvent.class, event -> {
                uno.events.UnoPenaltyEvent e = (uno.events.UnoPenaltyEvent) event;
                if (session.isLocalPlayer(e.getPlayer().getName())) {
        ...
        if (session.isHost()) {
            ...
            eventBus.subscribe(uno.events.UnoGracePeriodEvent.class, event
                    -> broadcast(MessageSerializer.serializeUnoGrace(
                            (uno.events.UnoGracePeriodEvent) event)));
    ...
    // ─────────────────────────────────────────────
    /**
     * Recibe un mensaje y lo enruta según el rol: Host valida y reenvía, Peer
     * publica en bus.
     *
        ...
        switch (type) {
            ...
            case MessageSerializer.TYPE_COLOR_CHOSEN:
                ...
                if (gameModel != null && gameModel.getGameState() != null) {
                    ...
                    if (chosenColor != null) {
                        uno.model.Card.Color chosen = parseCardColor(chosenColor);
    ...
    private void publishToLocalBus(String type, Map<String, String> fields) {
        switch (type) {
            ...
            case MessageSerializer.TYPE_LOBBY_STATE:
                ...
                if (newCapStr != null) {
                    try {
                        lobbyState.setCapacity(Integer.parseInt(newCapStr));
            ...
            case MessageSerializer.TYPE_GAME_OVER:
                ...
                eventBus.publish(GameEventFactory.gameOver(
                        new uno.model.Player(fields.get("winner"), "", false)));
                break;
            ...
            case MessageSerializer.TYPE_UNO_GRACE:
                ...
                eventBus.publish(new uno.events.UnoGracePeriodEvent(
                        new uno.model.Player(fields.get("player"), "", false)));
                break;
     ...
     */
    private uno.model.Card.Color parseCardColor(String s) {
        switch (s) {
            case "RED":
                return uno.model.Card.Color.RED;
            case "BLUE":
                return uno.model.Card.Color.BLUE;
            case "GREEN":
                return uno.model.Card.Color.GREEN;
            case "YELLOW":
                return uno.model.Card.Color.YELLOW;
            default:
                return uno.model.Card.Color.WILD;
        }
    }
